...
#Overview
#--------
#This file contains different models to the networks used 
#in the MAGIC implementation to reduce the training and 
...
class MAGICGen_Autoencoder(nn.Module):
    def __init__(self, context_size, particle_size, context_dependent, belief_dependent):

        super(MAGICGen_Autoencoder, self).__init__()
        print("Using autoencoder generator")
    ...
    def forward(self, c, x):
        ...
        if self.context_dependent or self.belief_dependent:
            ...
            x = F.relu(self.fc3(x))
            x_skip_1 = x = F.relu(self.fc4(x))
            x_skip_2 = x = F.relu(self.fc5(x))
            x = F.relu(self.fc6(x))
            x = F.relu(self.fc7(x) + x_skip_2)
            x = F.relu(self.fc8(x) + x_skip_1)
            x = F.relu(self.fc9(x))

            mean = self.fc10_mean(x)
            mean = mean.view((-1, NUM_CURVES, 2 * MACRO_CURVE_ORDER))
            mean = mean / (mean**2).sum(dim=-1, keepdim=True).sqrt()
            #print(f"Final mean shape: {mean.shape}")
...
class MAGICCritic_Autoencoder(nn.Module):
    def __init__(self, context_size, particle_size, context_dependent, belief_dependent):
        super(MAGICCritic_Autoencoder, self).__init__()
        print("Using autoencoder critic")
        self.context_size = context_size
        self.particle_size = particle_size
        self.context_dependent = context_dependent
        self.belief_dependent = belief_dependent
    ...
    def forward(self, c, x, action):
        ...
        x = F.relu(self.fc3(x))
        x_skip1 = x = F.relu(self.fc4(x))
        x_skip2 = x = F.relu(self.fc5(x))
        x = F.relu(self.fc6(x))
        x = F.relu(self.fc7(x)+x_skip2)
        x = F.relu(self.fc8(x)+x_skip1)
        x = F.relu(self.fc9(x))
        x = self.fc10(x)
...
class MAGICGen_Autoencoder_DriveHard(nn.Module):
    def __init__(self, macro_length, context_dependent, belief_dependent):

        super(MAGICGen_Autoencoder_DriveHard, self).__init__()
    ...
    def forward(self, c, x):
        ...
        if self.context_dependent or self.belief_dependent:
            ...
            x = F.relu(self.fc1(x))
            x_skip_1 = x = F.relu(self.fc2(x))
            x_skip_2 = x = F.relu(self.fc3(x))
            x = F.relu(self.fc4(x))
            x = F.relu(self.fc5(x)+x_skip_2)
            x = F.relu(self.fc6(x)+x_skip_1)
            x = F.relu(self.fc7(x))
...
class MAGICGen_Encode_RNN(nn.Module):
    def __init__(self, context_size, particle_size, context_dependent, belief_dependent):
        ...
        if self.context_dependent or self.belief_dependent:
            ...
            self.fc10_concentration = nn.Linear(32, NUM_CURVES)
            torch.nn.init.constant_(self.fc10_concentration.bias, 100)
...
class MAGICGen_Simple_RNN(nn.Module):
    def __init__(self, context_size, particle_size, context_dependent, belief_dependent):

        super(MAGICGen_Simple_RNN, self).__init__()
        self.context_size = context_size
        self.particle_size = particle_size
        self.context_dependent = context_dependent
        self.belief_dependent = belief_dependent
