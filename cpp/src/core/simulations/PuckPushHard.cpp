...
const boost::geometry::model::ring<vector_t, true, false> BOARD_REGION_BOT_BOOST(
  PuckPushHard::BOARD_REGION_BOT.begin(), PuckPushHard::BOARD_REGION_BOT.end());

const boost::geometry::model::ring<vector_t, true, false> BOARD_REGION_PUCK_BOOST(
  PuckPushHard::BOARD_REGION_PUCK.begin(), PuckPushHard::BOARD_REGION_PUCK.end());

PuckPushHard::Action PuckPushHard::Action::Rand() {
  return {std::uniform_real_distribution<float>(0, 2 * PI)(Rng())};
}

list_t<list_t<PuckPushHard::Action>> PuckPushHard::Action::CreateHandcrafted(size_t length) {
...
list_t<list_t<PuckPushHard::Action>> PuckPushHard::Action::Deserialize(const list_t<float>& params, size_t macro_length) {
  return StandardMacroActionDeserialization<PuckPushHard::Action>(params, macro_length);
}

uint64_t PuckPushHard::Observation::Discretize() const {
  list_t<int> data {
...
PuckPushHard::PuckPushHard() : step(0), _is_terminal(false), _is_failure(false) {

}

PuckPushHard PuckPushHard::CreateRandom() {
...
PuckPushHard PuckPushHard::SampleBeliefPrior() {
  PuckPushHard sim;
...
float PuckPushHard::Error(const PuckPushHard& other) const {
  float error = 0;
...
float PuckPushHard::BestReward() const {
...
std::tuple<PuckPushHard, float, PuckPushHard::Observation, float> PuckPushHard::Step(
      const PuckPushHard::Action& action, const PuckPushHard::Observation* observation) const {
  if (_is_terminal) { throw std::logic_error("Cannot step terminal simulation."); }

  PuckPushHard next_sim = *this;
  float reward;
...
template std::tuple<PuckPushHard, float, PuckPushHard::Observation, float> PuckPushHard::Step<true>(
    const PuckPushHard::Action& action, const PuckPushHard::Observation* observation) const;
template std::tuple<PuckPushHard, float, PuckPushHard::Observation, float> PuckPushHard::Step<false>(
    const PuckPushHard::Action& action, const PuckPushHard::Observation* observation) const;

/* ====== Serialization functions ====== */

void PuckPushHard::Encode(list_t<float>& data) const {
  data.emplace_back(static_cast<float>(step));
  bot_position.Encode(data);
  puck_position.Encode(data);
}

void PuckPushHard::EncodeContext(list_t<float>& data) {
  if constexpr (RANDOMIZE_GOAL) {
    GOAL.Encode(data);
  }
}

cv::Mat PuckPushHard::Render(const list_t<PuckPushHard>& belief_sims,
