...
public:
  ...
  static constexpr float BOT_OBSERVATION_NOISE = 5;
  static constexpr float PUCK_RADIUS = 16;
  static constexpr float PUCK_OBSERVATION_NOISE = 7; //observation noise (variance) when outside of the paper
  static constexpr float PUCK_FLICKER_PROBABILITY = 0.1f; //probability of not receiving observation when outside of the paper
  static constexpr float PUCK_BOT_ROLL_CONSTANT = 0.01f;
  static constexpr float PUCK_BOT_ROLL_CONSTANT_NOISE = 0.005f;
  static constexpr float PUCK_BOT_ROLL_DISPLACEMENT = 0.0f;
  static constexpr float PUCK_BOT_ROLL_DISPLACEMENT_NOISE = 0.05f;
  static constexpr float GOAL_RADIUS = 60.0f;

  static constexpr vector_t BOT_START_POSITION = {200, 299};
  // Randomization over initialization and context.
  static constexpr vector_t PUCK_START_POSITION = {350, 299};
  ...
  static constexpr array_t<array_t<vector_t, 2>, 3> NOISY_REGIONS{
      ...
      array_t<vector_t, 2>{
        vector_t(490 - PUCK_RADIUS, 40 - PUCK_RADIUS),
        vector_t(570 + PUCK_RADIUS, 558 + PUCK_RADIUS)
      },
      array_t<vector_t, 2>{
        vector_t(660 - PUCK_RADIUS, 40 - PUCK_RADIUS),
        vector_t(740 + PUCK_RADIUS, 558 + PUCK_RADIUS)
      },
      array_t<vector_t, 2>{
        vector_t(830 - PUCK_RADIUS, 40 - PUCK_RADIUS),
        vector_t(910 + PUCK_RADIUS, 558 + PUCK_RADIUS)
      },
  };
  ...
  PuckPushHard();
  static PuckPushHard CreateRandom();

  /* ====== Belief related functions ====== */
  static PuckPushHard SampleBeliefPrior();
  float Error(const PuckPushHard& other) const;
  ...
  std::tuple<PuckPushHard, float, Observation, float> Step(
  ...
  cv::Mat Render(const list_t<PuckPushHard>& belief_sims,
